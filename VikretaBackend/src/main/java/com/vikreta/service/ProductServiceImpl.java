...
public class ProductServiceImpl implements ProductService {
	...
	public List<ProductResponseDTO> getAllProducts() {
		return productDao.findAll().stream().map((product)-> modelMapper.map(product,ProductResponseDTO.class))
	...
	public List<ProductResponseDTO> getProductsByUser(Long userId) {
		...
		return productDao.findBySeller(user).stream().map((product) -> modelMapper.map(product,ProductResponseDTO.class ))
				.collect(Collectors.toList());
	}
